...
- **`processes`** replaces `asset__candidates`: fundamental carrier-flow capabilities defined by mathematical role (`boundary_exchange`, `conversion`, temporal / spatial / delayed / buffered / discrete / state-changing transfer, `explicit_demand`) rather than by named asset types. `explicit_demand` splits into `inelastic`, `price_responsive`, and `mobile_storage` (EV fleets / V2G: demand with embedded storage whose grid attachment follows a travel pattern). `system__carriers` is gone: multi-carrier coupling is a property of `conversion`, not a separate feature.
